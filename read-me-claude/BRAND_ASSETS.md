...
The bot avatar file is ready for BotFather; it has not been applied to Telegram. The reference description is “Railbird · raise a poker player.” The actual working username and public domain remain deployment configuration, not the sample handle in the archive. B13 does not replace the functional empty guest room: Jens explicitly requires the table and first-agent flow. Batch37 implements B15's header peek/look and delayed activity badge; B12 loading remains its separate correct animation. Batch38 replaces the old welcome-button label and hard-coded bot destinations. Entry/notification links use MINI_APP_URL, then TELEGRAM_BOT_USERNAME, then PUBLIC_BASE_URL; missing configuration omits a false link. The existing bot's account name/avatar/description must still be applied externally. DESIGN_GAP.md records the remaining external checks.

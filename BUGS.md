...
Named BUG headings: 170, including the two historically reused BUG-121 headings. Historical reports are retained; an OPEN section heading alone does not override an entry's verified FIXED status.

### BUG-178 — the host's Home wire omits a visitor's saved appearance — OPEN, projection audit
...
### BUG-177 — policy remarks still repeat within a speaker during a short game — OPEN, observed dialogue debt
...
### BUG-176 — the TV's lower opponent is hard to distinguish in a two-seat game — OPEN, legibility investigation
...
### BUG-175 — a seated visitor keeps the reading prop from his idle routine — OPEN, actual signed household observation
...
### BUG-174 — Home's collapsed conversation can name the wrong speaker — OPEN, isolated next-batch repair
...
### BUG-173 — ordinary wins retain the old compact result instead of the C8 card — OPEN, current-reference and actual-browser audit

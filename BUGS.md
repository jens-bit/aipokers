...
Named BUG headings: 184, including the two historically reused BUG-121 headings. Historical reports are retained; an OPEN section heading alone does not override an entry's verified FIXED status.

### BUG-192 — desktop away-agent Open does nothing when the agent has no live seat — OPEN, separately reproduced during190
...
### BUG-191 — the external Telegram SDK delays the first screen during normal entry — OPEN, measured startup dependency
...
### BUG-190 — the away-agent plate still uses the older long-name formatter — OPEN, isolated source audit/port for batch52
...
### BUG-189 — the Home request omits its authored speaker portrait and inline identity name — OPEN, isolated source port for batch52
...
### BUG-188 — away-agent frame numbers inherit Inter instead of the authored mono face — OPEN, isolated source port for batch52

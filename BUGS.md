...
Named BUG headings: 162, including the two historically reused BUG-121 headings. Historical reports are retained; an OPEN section heading alone does not override an entry's verified FIXED status.

### BUG-170 — policy action remarks reuse one voice across distinct agent natures — OPEN, normal-paced playtest reproduction
...
### BUG-169 — live Home TV miniature omits the authored occupants and caption — OPEN, current-reference visual audit
...
### BUG-168 — Home does not consume the live floor updates used by its TV — OPEN, signed built-browser reproduction

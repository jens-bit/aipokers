...
Named BUG headings: 171, including the two historically reused BUG-121 headings. Historical reports are retained; an OPEN section heading alone does not override an entry's verified FIXED status.
...
### BUG-178 — the host's Home wire omits a visitor's saved appearance — FIXED in isolated81d2f0b; reserved for batch50
...
### BUG-177 — policy remarks still repeat within a speaker during a short game — FIXED in isolated38d486b; reserved for batch50
...
**Remaining scope:** batch49's6d4572f repairs all eight listed active desktop-stage, tile and floor-chip calls with canonical money()/group(), preserving unknown/zero and currency-symbol rules. The native Swedish-locale browser and full combined gates pass. StandupPanel's signed total, desktop All time and current action amounts are being audited separately; unused PlayerSeat remains latent. This entry stays OPEN until the remaining active paths are verified.

...
**Contract riders (3a).** The ATTR-2 client was written against fields the server did not serve. Now served, client untouched: `nature.builtFor` / `nature.struggle` (the zero-sum pair in words, for the birth card's BUILT FOR / WILL STRUGGLE rows); `firstWords` (his first sentence at the reveal, a template per nature, no model call); `natureHint` on `POST /api/agents/chat` (the nature the ladder *would* pick from the owner's turns so far — two keyword signals minimum, null until then, a NAME only so a guess can never be mistaken for a birth); `sessionHands` and `effectiveAttrs` on the list projection; `attrLog` on both the list and `GET /api/agents/:id`, which now takes auth and scrubs hole cards for a non-owner exactly as `/:id/flagged` does. `ensureAttributes` backfills the nature's words onto agents born before them — same nature, new prose, never a re-roll.

**Growth (3b).** Ticks are drawn once per attribute at the END of a session from that attribute's own evidence, so an agent grows from how he was deployed rather than from how long the app was open. The curve is the ref's: free below the band, 0.35 of full speed at the low edge, a crawl through the lower half, near-zero in the top half, over at hi; a perfect session tops out at a coin flip. Evidence is counted on the live path in `table.js` by the pure rules in `attributes.js` — reads formed (once per opponent), decisions where the arithmetic held, big folds made correctly under a deviation die, beats survived without tilting, bluffs through uncalled, hands played. Every tick is an `attrLog` entry with a cause in his world.

**The scouted ceiling (3b).** Bands narrow from hands played, never wins, at 120 / 500 / 2000 hands to widths 24 / 8 / 2. The truth each band closes toward is derived from the agent's id and never stored — an exact potential in the record is one careless projection away from the screen. Bands may only close, never leave the birth band, and never claim a ceiling he has already passed. Each narrowing writes an `attrLog` entry with cause `narrowed` and sets the transient `agent.narrowed` for the caret.

**Fatigue in the loop (3c).** The stage is written to the record when it changes; the crossing into `worn` writes one moment in his own voice and never notifies; session end resets stage, hands and the latch. No mood penalty — fatigue is STATE, and all it moves is effective FOCUS and DISCIPLINE.

**attrCosts (3c).** Flagged hands carry `[{ key, line, street?, cost? }]`. FOCUS when the misjudgment was ≥ 5 points AND crossed the price he was offered; DISCIPLINE when the die gave him licence on an out-of-range hand and he took it; COMPOSURE when he played it steaming; READS when he was briefed and folded at a price that called. When the same mechanism won the pot it still gets a line with `cost:false` — an attribute that only ever appears when it costs money is a scold, not a character.

**Verification (3d).** `node scripts/verify-growth.js` plays 600 hands across twelve sessions offline in about a second — real engine, real handler, real evidence rules, real growth — and prints the session table, the growth log, the band history and what his attributes cost him. It deletes `ANTHROPIC_API_KEY` before importing the agent module and asserts it stayed gone, so it can never bill; `src/test/verifyScripts.test.js` discovers it, so it runs on every `npm test` and every CI push. Sample (seed 7): 32 ticks, 12 narrowings, bands 30 → 24 → 8, READS 43→49, FOCUS 38→46, DISCIPLINE 32→41, COMPOSURE 23→31. `arena --attributes grow` seats a day-one agent instead of a dial setting and prints his birth, evidence and ticks.

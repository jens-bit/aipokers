...
for (const height of [590, 844]) {
  test(`BUG-181: Home glass preserves the want, conversation and table taps at390x${height}`, async ({ page }) => {
    ...
    // BUG-184 now ports the authored22px actor line: the room band is78px.
    // BUG-181's glass, composer, want, POST and table guarantees remain exact.
    expect(box).toMatchObject({ x: 0, width: 390, height: 78, y: height - 78 });

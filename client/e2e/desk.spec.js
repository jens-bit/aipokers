...
test.describe('BUG-125: authored casino camera with real touch input',()=>{
  ...
  // SPEC-1 · TESTING LAW #6 — BUG-219, and this test is what found it.
  //
  ...
  // until BUG-219 lands. 1440 still runs and still covers the camera. Do not
  // loosen the 65% — it is the whole claim.
  for(const size of [{width:390,height:844},{width:390,height:590},{width:1440,height:900}])
    (size.width<1100?test.fixme:test)((size.width<1100?'BUG-219: ':'')+'pinch, return, then Watch at '+size.width+'x'+size.height,async({page,context})=>{
    const errors=[];page.on('pageerror',e=>errors.push(e.message));

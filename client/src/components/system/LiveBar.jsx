...
export function LiveBar({ table, blinds, pot, equity, action, timer = 12, board = [], street,
  ...
  return (
    <div style={{
      ...
      ...(strip ? {} : { borderBottom: '1px solid color-mix(in srgb, var(--accent) 24%, transparent)' }),
      boxShadow: 'inset 0 1px 0 color-mix(in srgb, var(--accent) 18%, transparent), 0 6px 14px rgba(0,0,0,0.35)',
      cursor: 'pointer',
    }}>
      {/* ── Top row ── */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 14px 0' }}>
        ...
        <span style={{
          ...
          color: faceDown ? 'var(--text-muted)' : 'var(--accent)',
        ...
        {!faceDown && typeof timer === 'number' && (
          <span style={{
            ...
            color: timerCritical ? 'var(--error)' : 'var(--text-secondary)', fontWeight: 600,
          }}>{timer}s</span>
        )}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '6px 14px 9px' }}>
        ...
        {hole && (
          <>
            ...
            <div style={{ width: 1, height: 22, background: 'var(--edge)', flexShrink: 0 }} />
          </>
        )}
        ...
        {note ? (
          <>
            <div style={{ width: 1, height: 22, background: 'var(--edge)', flexShrink: 0 }} />
            <div style={{ fontSize: 12, color: 'var(--text-muted)', fontStyle: 'italic', minWidth: 0 }}>{note}</div>
            <div style={{ flex: 1 }} />
          </>
        ) : (
          <>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              <div>
                <span style={{
                  ...
                  fontWeight: 700, color: 'var(--text-primary)', fontVariantNumeric: 'tabular-nums',
                }}>${pot}</span>
              </div>
            </div>
            <div style={{ width: 1, height: 22, background: 'var(--edge)', flexShrink: 0 }} />
            <div style={{ minWidth: 0 }}>
              ...
              <div>
                <span style={{
                  fontFamily: '"JetBrains Mono",ui-monospace,monospace', fontSize: 13,
                  fontWeight: 700, color: 'var(--accent)', fontVariantNumeric: 'tabular-nums',

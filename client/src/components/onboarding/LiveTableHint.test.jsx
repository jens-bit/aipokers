...
const guide = vi.hoisted(() => ({ stage: 'live', dismiss: vi.fn() }));
vi.mock('./FirstRunGuide.jsx', () => ({ useFirstRunGuide: () => guide }));

beforeEach(() => {
  guide.stage = 'live'; guide.dismiss.mockClear();
  telegram.signIn();
  ...
  vi.spyOn(HTMLElement.prototype, 'getBoundingClientRect').mockImplementation(function () {
    ...
    const [left, top, width, height] = card ? [120 + cardIndex * 46, 210, 44, 64]
...
it.each([['phone', phone], ['desktop', desk], ['desktop casino', casino]])('CONTEXT-HINT-1: %s points at the existing board and Got it only dismisses the guide', async (_, table) => {
  ...
  expect(hint).toHaveTextContent('You’re watching; players use these shared cards.');
  expect(screen.getByTestId('context-hint-target')).toHaveStyle({ left: '120px', width: '228px', height: '64px' });
  fireEvent.click(within(hint).getByRole('button', { name: 'Got it' }));
  expect(guide.dismiss).toHaveBeenCalledOnce();

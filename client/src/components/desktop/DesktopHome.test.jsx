...
describe('DesktopHome roster', () => {
  ...
  it('sends the Telegram initData header when reading the roster (FLOOR-3)', async () => {
    renderHome();
    await waitFor(() => {
      const call = fetchMock.calls.find((c) => c.url.includes('/api/agents?'));
      expect(call?.headers['x-telegram-init-data']).toBeTruthy();
    });
  });

  it('opens on the standup panel, not on a thread', async () => {
    renderHome();
    await waitFor(() => expect(panelHead('Standup')).toBe(true));
  });

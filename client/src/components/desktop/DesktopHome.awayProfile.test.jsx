...
it.each([false, true])('BUG-192: Open him selects the full owned profile by id, including an owned visitor (%s)', async visiting => {
  ...
  const profile = await screen.findByRole('region', { name: "Professor Oak's profile", exact: true });
  expect(within(profile).getByText('Professor Oak', { exact: true })).toBeInTheDocument();
  expect(screen.queryByRole('region', { name: "Professor Plum's profile" })).toBeNull();
  ...
  const reopened = await screen.findByRole('region', { name: "Professor Oak's profile" });
  await userEvent.click(within(reopened).getByRole('button', { name: 'Back to chat' }));
  expect(within(screen.getByRole('region', { name: "Professor Oak's room" })).getByRole('textbox')).toHaveValue('Keep this unsent draft');
  await userEvent.click(screen.getByTestId('home-frame-plum'));
  expect(await screen.findByRole('region', { name: "Professor Plum's profile" })).toBeInTheDocument();
  await userEvent.click(screen.getByTestId('home-frame-oak'));
  await userEvent.click((await screen.findByRole('region', { name: "Professor Oak's profile" })).querySelector('[aria-label="Back to chat"]'));
  expect(within(screen.getByRole('region', { name: "Professor Oak's room" })).getByRole('textbox')).toHaveValue('Keep this unsent draft');
  await userEvent.keyboard('{Escape}');
  expect(await screen.findByTestId('room-thread')).toBeInTheDocument();
});

it('BUG-192: a guest-only room projection cannot open an owned same-name private profile', async () => {
...
it('BUG-192: removing the selected owner agent returns to the room', async () => {
  mount();
  await userEvent.click(await screen.findByTestId('home-frame-oak'));
  await screen.findByRole('region', { name: "Professor Oak's profile" });
  fetchMock.route('/api/agents', { agents: [plum] });
  fireEvent.focus(window);
  await waitFor(() => expect(screen.queryByRole('region', { name: "Professor Oak's profile" })).toBeNull());
  expect(await screen.findByTestId('room-thread')).toBeInTheDocument();
});

it('BUG-192: returning from an opened profile retains a successful funding result', async () => {

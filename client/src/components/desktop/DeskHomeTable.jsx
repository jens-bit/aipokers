...
export function DeskHomeTable({ game, mySeat, seated, legalActions, onAct, lastDecision, agents, connection, onBack,
  ...
  return <>
    <WatchGuide rootRef={viewport} game={game} heroSeat={Number.isInteger(mySeat) && mySeat >= 0 ? mySeat : game?.seats?.findIndex(Boolean)}
      blocked={seated || !!sessionEnd || !!error || selectedSeat != null || connection === 'reconnecting'}/>
    <div className="dsk-stage dsk-stage--felt dsk-stage--home-game">
      ...
      <section className={'dsk-home-table' + (seated ? ' is-seated' : '')} data-testid="desk-home-table" aria-label="The kitchen table" style={{width:900*scale,height:648*scale}}>
        ...
        {!seated && <span className="dsk-casino-table__name" data-watch-status style={{background:'var(--v5-panel)',padding:'2px 6px',borderRadius:4,pointerEvents:'auto'}}>Watching the kitchen table</span>}

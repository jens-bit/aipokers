...
export function AgentView({ agent, mood, heat, chat, loading, draft, setDraft, send, inputRef, feedRef, onBack, onOpenProfile, onDeploy, onWatch, onCarry, onReplay, onAccept, accepting }) {
  ...
  const lastLine = chat.some(m => m.role === 'user') ? [...chat].reverse().find(m => m.role === 'assistant' && !m.error)?.content : null;
  ...
  return <section className="agent-view" aria-label={`${agent.name}'s room`}>
    <header className="agent-view__header">
      <button className="agent-view__back" type="button" aria-label="Back" onClick={onBack}><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden><path d="M15 18l-6-6 6-6"/></svg></button>
    ...
    <div className="agent-view__stage" data-testid="agent-stage">
      ...
      <div className="agent-view__body"><NamePill name={agent.name} nickname={agent.nickname} fatigue={agent.fatigue} heat={heat} accent="#EDEDED"/><div className="agent-view__breath">{face(178)}<svg className="agent-view__hands" width="178" height="178" viewBox="0 0 80 80" aria-hidden>{ghostHands({ pose: 'rest', size: 178, grip: SEAT_GRIP })}</svg></div></div>
    ...
    {error && !funding && <div className="agent-view__error" role="alert">{error}</div>}
    <div ref={feedRef} className="agent-view__thread">
      ...
      {chat.map(msg => {
        ...
        return <div key={msg._id} className={`agent-view__line${mine ? ' is-mine' : ''}`}>
          ...
          <div className="agent-view__text">{msg.role === 'proposal' ? <>{msg.proposal.reasoning}<div className="agent-view__proposal"><button type="button" disabled={accepting} onClick={() => onAccept(msg._id)}>Accept change</button><button type="button" onClick={() => inputRef.current?.focus()}>Discuss</button></div></> : msg.content}</div>
        </div>;
      })}
